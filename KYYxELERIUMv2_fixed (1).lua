...
-------------------- hide-frames helper --------------------
-- matches any frame whose name contains these keywords (case-insensitive)
local frameKeywords = {"strength", "durability", "agility"}
local framesHidden = false
local hiddenSet = {}   -- [GuiObject] = original Visible state

local function nameMatches(n)
    n = string.lower(n)
    for _,kw in ipairs(frameKeywords) do
        if string.find(n, kw, 1, true) then return true end
    end
    return false
end

local function setFramesVisible(vis)
    local pg = Player:FindFirstChild("PlayerGui") or Player:WaitForChild("PlayerGui")
    for _,d in ipairs(pg:GetDescendants()) do
        if d:IsA("GuiObject") and nameMatches(d.Name) then
            if not vis then
                -- remember it and hide it (plus hide direct parent frame so borders/bg go too)
                hiddenSet[d] = true
                d.Visible = false
                local p = d.Parent
                if p and p:IsA("GuiObject") then p.Visible = false end
            else
                if hiddenSet[d] then
                    d.Visible = true
                    local p = d.Parent
                    if p and p:IsA("GuiObject") then p.Visible = true end
                    hiddenSet[d] = nil
                end
            end
        end
...
-- hide frames the moment the game spawns new ones
Player:WaitForChild("PlayerGui").DescendantAdded:Connect(function(d)
    if framesHidden and d:IsA("GuiObject") and nameMatches(d.Name) then
        d.Visible = false
    end
end)

-- keep re-hiding in case the game re-enables them (stats update loop etc.)
task.spawn(function()
    while true do
        if framesHidden then setFramesVisible(false) end
        task.wait(0.2)
    end
end)
...
RebirthTab:AddToggle("Hide Frames",false,function(s)
    if s then hideFrames() else showFrames() end
end)
